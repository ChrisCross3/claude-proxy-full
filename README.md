...
- **OpenRouter masking** — `src/adapter/openrouter-normalize.ts` strips the `anthropic/` model prefix and lifts `reasoning` (top-level *or* `extra_body`) so callers configured for OpenRouter (e.g. Hermes) can drive the proxy unchanged.
